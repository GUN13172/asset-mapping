...
async fn main() {
    ...
    // 从环境变量或配置文件读取API密钥
    let api_key = std::env::var("HUNTER_API_KEY").unwrap_or_else(|_| {
                    ...
                    match serde_json::from_str::<Value>(&text) {
                        Ok(json) => {
                            ...
                            if code == 200 {
                                ...
                                println!("✗ API密钥无效");
                                if let Some(msg) =
                                    json["message"].as_str().or_else(|| json["msg"].as_str())
                                {
                                    println!("错误信息: {}", msg);
                                }
                            }
                        ...
                        Ok(json) => {
                            ...
                            if code == 200 {
                                ...
                                if let Some(data) = json["data"].as_object() {
                                    ...
                                    if let Some(consume) =
                                        data.get("consume_quota").and_then(|v| v.as_str())
                                    {
                                        println!("消耗积分: {}", consume);
                                    }
                                    if let Some(rest) =
                                        data.get("rest_quota").and_then(|v| v.as_str())
                                    {
                                        println!("剩余积分: {}", rest);
                                    }
                                }
